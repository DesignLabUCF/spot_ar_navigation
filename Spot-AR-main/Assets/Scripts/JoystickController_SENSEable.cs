...
namespace Microsoft.MixedReality.Toolkit.Experimental.Joystick
{
    ...
    /// </summary>
    public class JoystickController_SENSEable : MonoBehaviour
    {
        /*
        [Experimental]
        ...
        */
        ...
        private void Start()
        {
            ...
            startRotation = grabberVisual.transform.localEulerAngles;
        ...
        private void Update()
        {
            if (!isDragging)
            {
                ...
                if (grabberVisual != null)
                {
                    ...
                    //grabberVisual.transform.localEulerAngles = Vector3.Lerp(grabberVisual.transform.localEulerAngles, startRotation, Time.deltaTime * ReboundSpeed);
                    grabberVisual.transform.localEulerAngles = new Vector3(0f, Mathf.LerpAngle(grabberVisual.transform.localEulerAngles.y, 0f, Time.deltaTime * ReboundSpeed), 0f);
                }
            }
        ...
        */
        ...
        public Vector3 GetJoyStickMove()
        {
            //return joystickGrabberPosition / ((float)joystickVisualMaxRotation);
            return joystickVisualRotation / ((float)joystickVisualMaxRotation);
        }

        public float GetJoyStickRotation()
        {
            if(grabberVisual.transform.localEulerAngles.y > 180.0f)
                return -(360.0f - grabberVisual.transform.localEulerAngles.y);
            else
                return grabberVisual.transform.localEulerAngles.y;
        }
    }
}
