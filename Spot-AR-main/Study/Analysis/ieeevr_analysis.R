...
library(ARTool) # For ART anova - https://cran.r-project.org/web/packages/ARTool/readme/README.html
...
exclude_subjects <- c("ExpertDwight",
                      "ExpertJohn",
                      "4",
                      "10",
                      "23",
                      ...
                      "34",
                      "35",
                      "36", # Movement data not collected properly
                      "45",
                      "46",
                      "47",
                      "48",
                      ...
                      "61",
                      "62"
                      ) # Subjects to exclude (noted in Log)
...
qualtrics <- qualtrics[!(qualtrics$Participant.ID %in% exclude_subjects),] # Keep only relevant subjects
...
log_data <- log_data[!(log_data$Participant.ID %in% exclude_subjects),] # Keep only relevant subjects
...
##### High Low Performers (Time)

add_high_low_performer_time <- function(data, progress_tracker_best, route)
{
  ## AR
  # Calculate median time and determine which subjects are below it
  median_time <- median(subset(progress_tracker_best, Route==route & Control=="AR")$Time)
  high_performing_participants <- subset(progress_tracker_best, Route==route & Control=="AR" & Time <= median_time)$Participant.ID
  # Check if each row is high performing
  col_ar <- data$Participant.ID %in% high_performing_participants
  
  ## Tablet
  # Calculate median time and determine which subjects are below it
  median_time <- median(subset(progress_tracker_best, Route==route & Control=="Tablet")$Time)
  high_performing_participants <- subset(progress_tracker_best, Route==route & Control=="Tablet" & Time <= median_time)$Participant.ID
  # Check if each row is high performing
  col_tablet <- data$Participant.ID %in% high_performing_participants
  
  # Add column to data
  data[paste0("High_Performing_", route)] <- col_ar | col_tablet
  return(data)
}
data <- add_high_low_performer_time(data, progress_tracker_best, "Forward")
data <- add_high_low_performer_time(data, progress_tracker_best, "Backward")
data <- add_high_low_performer_time(data, progress_tracker_best, "Combined")
data$High_Performing_All <- data$High_Performing_Forward & data$High_Performing_Backward & data$High_Performing_Combined

# Add back into best times dataset for easier plotting
forward_col <- (progress_tracker_best$Participant.ID %in% subset(data, High_Performing_Forward == TRUE)$Participant.ID) & (progress_tracker_best$Route=="Forward")
backward_col <- (progress_tracker_best$Participant.ID %in% subset(data, High_Performing_Backward == TRUE)$Participant.ID) & (progress_tracker_best$Route=="Backward")
combined_col <- (progress_tracker_best$Participant.ID %in% subset(data, High_Performing_Combined == TRUE)$Participant.ID) & (progress_tracker_best$Route=="Combined")
progress_tracker_best$High_Performer <- forward_col | backward_col | combined_col
...
# Maybe useful. Likely can just stick with ANOVA as it doesnt violate assumptions too bad
# model <- gam(Time ~ Control * Route, data = progress_tracker_best, family = gaussian())
# print(summary(model))

model <- art(progress_tracker_best$Time ~ as.factor(progress_tracker_best$Control) * as.factor(progress_tracker_best$Route))
anova(model)

# ANOVA - Considering Performance Classification - Actually, pretty pointless
res <- aov(Time ~ Control * Route * High_Performer, data = progress_tracker_best)
print(summary(res))
print(shapiro.test(x = residuals(object = res))) # Assumption of normality

model <- art(progress_tracker_best$Time ~ as.factor(progress_tracker_best$Control) * as.factor(progress_tracker_best$Route) * as.factor(progress_tracker_best$High_Performer))
anova(model)
...
# Jerk T-test
shapiro.test(progress_tracker_best$Zero_Crossings_Per_Second)
var.test(Zero_Crossings_Per_Second ~ Control, data = progress_tracker_best)
...
cohens_d(Zero_Crossings_Per_Second ~ Control,
         data = progress_tracker_best,
         var.equal = FALSE)
stat.desc(subset(progress_tracker_best, Control=="Tablet")$Zero_Crossings_Per_Second)
stat.desc(subset(progress_tracker_best, Control=="AR")$Zero_Crossings_Per_Second)

# Jerk with performance
res <- aov(Zero_Crossings_Per_Second ~ Control * High_Performer, data = progress_tracker_best)
print(summary(res))
#print(TukeyHSD(res, which = "Route"))
print(shapiro.test(x = residuals(object = res))) # Assumption of normality
print(EtaSq(res, type=2, anova=FALSE)) # Effect size
...
# pairwise_t_test( # Doing single test of differences instead to test individual control types
#   Score ~ Test,
...
#   p.adjust.method = "BH"
# )
trust_diff <- data.frame(Control=data$Control,
                         Complexity=data$Complexity,
                         Trust.Diff=data$Trust.Full.Score - data$Trust.Short1.Score)
shapiro.test(trust_diff$Trust.Diff)
var.test(Trust.Full.Score ~ Control, data = data)
res <- t.test(x=subset(trust_diff, Control=="AR")$Trust.Diff,
              alternative = "two.sided",
              paired = FALSE,
              var.equal = TRUE,
              conf.level = 0.95)
print(res)
stat.desc(subset(trust_diff, Control=="AR")$Trust.Diff)
res <- t.test(x=subset(trust_diff, Control=="Tablet")$Trust.Diff,
              alternative = "two.sided",
              paired = FALSE,
              var.equal = TRUE,
              conf.level = 0.95)
print(res)
stat.desc(subset(trust_diff, Control=="Tablet")$Trust.Diff)
...
plot_data <- melt(subset(data, select = c("Control", "Complexity", "Trust.Short1.Score", "Trust.Short2.Score", "Trust.Full.Score", "High_Performing_All")),
                  id = c("Control", "Complexity", "High_Performing_All"))
...
plot <- ggplot(plot_data,
  ...
  stat_summary(aes(x=variable, y=value, fill=Control),
               ...
               stroke=1.2,
               shape = 21) +
  facet_grid(.~factor(High_Performing_All),
             space="fixed",
             margins = "vs",
             labeller = as_labeller(c("TRUE"="High Performer on All Routes", "FALSE"="Other"))) +
  ylab("Trust Score") +
  scale_x_discrete(
    "",
    labels = c(
...
plot_data <- melt(subset(data, select = c("Control", "Complexity", "SUS.Score", "TLX.Mental_Demand", "TLX.Physical_Demand", "TLX.Temporal_Demand", "TLX.Performance", "TLX.Effort", "TLX.Frustration", "High_Performing_All")),
                  id = c("Control", "Complexity", "High_Performing_All"))
plot_data <- subset(plot_data, Complexity=="Inside")
...
expert_data <- data.frame(Participant.ID=c("John", "John", "John", "Dwight", "Dwight", "Dwight"),
                          Control=c("AR", "AR", "AR", "Tablet", "Tablet", "Tablet"),
                          Complexity=c("Inside", "Inside", "Inside", "Inside", "Inside", "Inside"),
                          Time=c(24.53427, 27.50284, 43.4076, 22.324484, 23.936066, 41.013262),
                          Route=c("Forward", "Backward", "Combined", "Forward", "Backward", "Combined"),
                          High_Performer=c(TRUE, TRUE, TRUE, TRUE, TRUE, TRUE)
                          )
expert_data2 <- expert_data
expert_data2$High_Performer <- FALSE
expert_data <- rbind(expert_data, expert_data2)
plot <- ggplot(plot_data, aes(x=factor(plot_data$Route, levels = c("Combined", "Backward", "Forward")),
  ...
  #              #width = 0.5,
  #              position=position_dodge(width=0.7)) +#,
  geom_point(data=subset(expert_data, Participant.ID=="John"),
             aes(x=factor(Route, levels = c("Combined", "Backward", "Forward")),
                 y=Time,
                 fill=Control),
             stat = 'identity',
  ...
  geom_point(data=subset(expert_data, Participant.ID=="Dwight"),
             aes(x=factor(Route, levels = c("Combined", "Backward", "Forward")),
                 y=Time,
                 fill=Control),
             stat = 'identity',
  ...
  facet_grid(.~factor(High_Performer),
             space="fixed",
             margins = "vs",
             labeller = as_labeller(c("TRUE"="High Performers", "FALSE"="Low Performers"))) +
