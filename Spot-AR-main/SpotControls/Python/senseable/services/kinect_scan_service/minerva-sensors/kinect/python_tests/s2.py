...
def k4a_sync_main():
    ...
    print(captures_list[0].color.data[:,:,1].shape)
    duration = 2
    fps = 25
    size = (captures_list[0].color.data.shape[1], captures_list[0].color.data.shape[0])
    out = cv2.VideoWriter('output.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, size, False)
    for _ in range(fps * duration):
        data = np.random.randint(0, 256, size, dtype='uint8')
        #print(data.shape)
        out.write(captures_list[0].color.data[:,:,2])
    out.release()
