...
manual_cancel = False
server_signal = False
kill_server_signal = False
### Imported script params
...
class FollowFiducial(object):
    ...
    def start(self):
        ...
        #while (self._attempts <= self._max_attempts) and (((datetime.now() - start_timestamp).total_seconds()) < 7.0): # Debug: Auto shut off after 7 seconds
        #while self._attempts <= self._max_attempts: # Original
        while (self._attempts <= self._max_attempts) and (not manual_cancel): # Original

            #if ((datetime.now() - start_timestamp).total_seconds()) < 7.0: # Debug - manual cancel test
            #    manual_cancel = True
...
'''
def start(robot, fiducial_follower, image_viewer, options):
    try:
        with Exit():
...
def start_listener(options):
    ...
    global manual_cancel
    global kill_server_signal

    # Listen for call to start
    robot_ip = str(options.spot_ip)
    ...
    while (not kill_server_signal):
        client, address = server.accept()
        print("Connection recieved from:", address)
        client.send("Connection made.\n".encode())
        request_type = client.recv(buffer_size).decode()
        print("Recieved request:", str(request_type))
        request_type = request_type.strip() == "True"
        try:
            if request_type == True:
                if currently_active == False:
                    print("True request recieved. Starting...")
                    # GIVE THE GO AHEAD
                    server_signal = True
                    #run_thread.start()
                else:
                    print("True request recieved, but fiducial follow is already running. Ignoring request...")
            else:
                print("False request recieved")
                # STOP
                manual_cancel = True
...
def run_ff(argv):
    ...
    parser.add_argument('--spot-ip', default=False,
                        help='Spots ID address.')
    ...
    '''
    run_thread = threading.Thread(target=start, args=(robot, fiducial_follower, image_viewer, options))
    while True:
        client, address = server.accept()
        print("Connection recieved from:", address)
        client.send("Connection made.\n".encode())
        ...
        try:
            if request_type == True:
                if currently_active == False:
                    print("True request recieved. Starting...")
                    #start(robot, fiducial_follower, image_viewer, options)
                    run_thread.start()
                else:
                    print("True request recieved, but fiducial follow is already running. Ignoring request...")
            else:
    ...
    #start(robot, fiducial_follower, option)

    server_thread = None
    try:
        ## Launch server thread. Wait for it to allow to continue
        server_thread = threading.Thread(target=start_listener, args=[options])
        server_thread.start()

        while True: # Keep running forever so Fiducial Follow can be started and stopped repeatedly

            while not server_signal: # Keep main thread waiting for signal
                pass
            server_signal = False # Reset to prep for next message from the server thread

            # Run normal Fiducial Follow script
            with Exit():
                bosdyn.client.util.authenticate(robot)
                robot.start_time_sync()
        ...
        if server_thread != None:
            kill_server_signal = True

    return False


if __name__ == '__main__':
    if not run_ff(sys.argv[1:]):
        sys.exit(1)
