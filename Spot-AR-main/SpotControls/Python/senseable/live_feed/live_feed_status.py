...
### SDL: Robot status
from robot_status import get_current_status
	...
	while running:
		...
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				running = False
			if event.type == pygame.VIDEORESIZE: # Available because RESIZABLE flag set
				display = (event.w, event.h)
				surface = pygame.display.set_mode(display, DEFAULT_DISPLAY_MODE)
				glViewport(0, 0, display[0], display[1]) # Set OpenGL view port so it doesn't snap back (https://stackoverflow.com/questions/64146091/how-to-make-a-resizeble-window-in-pygame-with-opengl)
		...
		### SDL: Add GUI
		current_status = get_current_status(robot_state_client, lease_client) # Returns an array of various robot status info. Check function for more information
		print(current_status)
		pass # TODO, put GUI stuff here. Is an OpenGL display so need to look up how to draw text to those in PyGame
...
def main():
	...
	sdk = bosdyn.client.create_standard_sdk('SENSEable_CameraStitch')
